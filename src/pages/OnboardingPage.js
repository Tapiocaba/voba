...
const OnboardingPage = ({ onOnboardingComplete }) => {
  ...
  useEffect(() => {
    if (showAlert) {
      ...
      setTimeout(() => setAlertVisible(false), 3000); // Alert fades out after 3 seconds
    }
    setShowAlert(false);
  }, [showAlert]);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="w-full max-w-md p-4 bg-white shadow-md rounded-lg">
        {alertVisible && (
          <div className={`fixed bottom-0 left-0 right-0 mx-auto mb-6 p-4 bg-red-200 border-red-400 text-red-700 rounded transition-opacity duration-500 ${showAlert ? 'opacity-100' : 'opacity-0'}`}>
            <span className="block sm:inline">Please fill out all fields before finishing.</span>
          </div>
        )}
        <div>
